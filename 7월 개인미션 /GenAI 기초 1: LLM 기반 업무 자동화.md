# LLM 모델 비교·선정 보고서


앞서, 이 자료는 Gemini 3 Flash, Claude Haiku 4.5, GPT-5.4 mini, Gemini 3.1 Pro, Claude Sonnet 4.6, GPT-5.4를 웹에서 2026.7.6 ~ 7.7 동안 사용했습니다.

## 모델 비교

Gemini 3 Flash, Claude Haiku 4.5, GPT-5.4 mini를 이들의 상위 모델인 Gemini 3.1 Pro, Claude Sonnet 4.6, GPT-5.4가 각각 평가하도록 해봤습니다.

선택한 과업은 실제 웹툰 제작사에서 진행할 법한 회의록을 가져와 봤습니다.

타겟 사용자는 프로젝트 매니저로 설정해 봤고 입력 데이터 형태는 결정사항, 해야 할 일, 위험 요소, 다음 일정 등을 요구했으며 표 형태로 회의록을 요약하게 만들었습니다. 입력 템플릿은 작가가 꼭 해야할 일을 필수 포함 항목에 등록 시켰습니다.

그렇게 최종 결과물을 각 상위 모델들에게 평가해달라 했을 때 아래와 같은 결과가 나왔습니다.


### Gemini 3.1 Pro

| 평가 축 | Claude Haiku | Gemini Flash | GPT-4o mini |
| :--- | :---: | :---: | :---: |
| **1. 정확성 (환각 통제)** | 5점 | 1점 | 2점 |
| **2. 지시 이해 (양식 준수)** | 3점 | 4점 | 5점 |
| **3. 안전성 (예외 처리)** | 5점 | 2점 | 3점 |
| **4. 실무 유용성** | 5점 | 2점 | 3점 |
| **총점 (20점 만점)** | **18점** | **9점** | **13점** |

1. 클로드 2. GPT 3. Gemini


### Claude Sonnet 4.6

| 평가 축 | Claude Haiku 4.5 | Gemini 3 Flash | GPT-5.4 Mini |
| :--- | :---: | :---: | :---: |
| **① 지침 이해도** | 2 | 5 | 5 |
| **② 표 완성도** | 1 | 5 | 4 |
| **③ 능동적 분석력** | 1 | 5 | 3 |
| **④ 확인 질문 품질** | 1 | 5 | 3 |
| **⑤ 근거 표기 일관성** | 1 | 5 | 5 |
| **⑥ 가독성/실용성** | 4 | 3 | 5 |
| **🏁 총점 (30점 만점)** | **10** | **28** | **25** |

1. Gemini 2. GPT 3. 클로드


### GPT-5.4

| 평가 축 | Claude Haiku 4.5 | GPT-5.4 mini | Gemini 3 Flash |
| :--- | :---: | :---: | :---: |
| **1. 정확성(사실 통제)** | 4/5 | 2/5 | 2/5 |
| **2. 안전성(환각 방지)** | 5/5 | 2/5 | 2/5 |
| **3. 형식/구조 완성도** | 3/5 | 4/5 | 5/5 |
| **4. 지시 준수도** | 4/5 | 3/5 | 3/5 |
| **총점** | **16/20** | **11/20** | **12/20** |

1. 클로드 2. Gemini 3. GPT




```text
따라서 Claud haiku 4.5 가 가장 유용한 AI로 판별이 났습니다.
```




## 시스템 설계 문서

아무런 지시 없이 그냥 회의록만 올리고 정리를 부탁했을 때 이런 식으로 답변이 왔습니다. (v1)

# 📊 「붉은 달의 계승자」 웹툰 런칭 회의 분석

## 1. 프로젝트 현황 분석

| 구분 | 내용 |
|------|------|
| ✅ 진행 현황 | 원고 진행률 8화 완료 → 런칭 일정 안정적 |
| ✅ 편집 상태 | 큰 오류 없음 → 품질 관리 양호 |
| ✅ 팀 협업 | 작가·PD·마케팅·편집이 유기적으로 소통 |
| ⚠️ 개선 사항 | 1화 초반 전개 속도 조절 필요 |
| 원인 | 세계관 설명이 많아 독자 진입장벽 발생 |
| 해결 방향 | 감정선 중심으로 재구성 |

---

## 2. 전략적 의사결정 분석

### 📈 마케팅 전략

| 단계 | 전략 | 목적 |
|------|------|------|
| 런칭 2주 전 | 티저 이미지 순차 공개 | 기대감 형성 |
| 런칭 초기 | SNS 캐릭터 소개 + 숏폼 영상 | 독자 유입 |
| 지속 운영 | 작가 인터뷰, 비하인드 콘텐츠 | 신뢰도 및 관심도 향상 |

### 💡 핵심 인사이트

| 항목 | 내용 |
|------|------|
| 데이터 기반 의사결정 | 경쟁작 분석 활용 |
| 모바일 최적화 | 말풍선 배치 수정으로 UX 개선 |
| 초기 목표 | 조회수 ❌ → 구독자 확보 ✅ |

---

## 3. 위험 요소 및 기회 요소

### ⚠️ 잠재적 리스크

| 항목 | 내용 |
|------|------|
| 일정 관리 | 1화 수정 후 플랫폼 검수 시간 필요 |
| 품질 관리 | 과도한 수정 시 전체 일정 지연 가능 |
| 마케팅 | 첫 주 광고 성과에 따라 전략 변경 필요 |

### ✅ 강점

| 항목 | 내용 |
|------|------|
| 팀 협업 | 각 부서가 구체적인 의견 제시 |
| 독자 중심 사고 | 감정선 및 모바일 환경 고려 |
| 운영 전략 | 댓글·리뷰 분석 후 시즌 운영 반영 |

---

## 4. 회의 효과성 평가

### 👍 잘된 점

| 항목 | 내용 |
|------|------|
| 역할 분담 | 담당자의 역할이 명확함 |
| 실행 계획 | 구체적인 실행 방안 제시 |
| 피드백 | 독자 의견을 반영하는 시스템 구축 |

### 🔧 개선할 점

| 항목 | 내용 |
|------|------|
| 일정 관리 | 1화 수정 완료 시점이 명확하지 않음 |
| 목표 설정 | 구독자 확보 목표 수치 필요 |
| 리스크 대응 | 광고 성과 저조 시 대안 전략 필요 |

---

# 📌 결론

| 평가 | 내용 |
|------|------|
| 종합 평가 | 체계적이고 데이터 기반의 런칭 전략을 수립한 회의 |
| 강점 | 초기 목표 설정과 독자 반응 모니터링 계획이 명확함 |
| 보완점 | 성공 지표(KPI)와 일정 관리 기준을 더욱 구체화하면 효과가 향상될 것으로 예상 |

<img width="940" height="1005" alt="스크린샷 2026-07-06 113341" src="https://github.com/user-attachments/assets/253c9994-4785-4148-a78e-e97e08e78f88" />
<img width="938" height="1006" alt="스크린샷 2026-07-06 113355" src="https://github.com/user-attachments/assets/3205b24b-6e67-4dd6-8d8c-3757aa3adeb2" />

이후

<img width="514" height="386" alt="스크린샷 2026-07-06 113808" src="https://github.com/user-attachments/assets/aa32c07d-5eda-49b1-bf4e-63f58978922b" />

이런 식으로 자동화 과업 및 페르소나 시스템 프롬프트를 설계 했습니다.

이 후엔

<img width="802" height="152" alt="스크린샷 2026-07-06 113915" src="https://github.com/user-attachments/assets/42537d3e-70ab-4d5c-a855-187df3896ef5" />

이런 식으로 Few shot으로 점검을 해줬더니 최종적으로는


# 📊 웹툰 「붉은 달의 계승자」 프로젝트 분석

## ① 결정사항

| 항목 | 내용 | 근거 |
|------|------|------|
| 런칭 일정 | 변동 없음 (예정대로 진행) | 김민수 PD 공유 |
| 1화 수정 방향 | 캐릭터 관계 자연스럽게 + 주인공 감정선 중심 | 정하늘 작가, 이지은 팀장 의견 |
| 초기 KPI | 조회수 ✗ → 구독자 확보 우선 | 참석자 전원 동의 |
| 마케팅 시작 시점 | 런칭 2주 전부터 티저 공개 | 이지은 팀장 제안 |
| 첫 주 마케팅 전략 | SNS 광고 예산 집중 운영 | 참석자 전원 동의 |
| 독자 피드백 관리 | 댓글·리뷰 데이터 매일 취합 → 작가/PD 공유 | 참석자 전원 동의 |

---

## ② 해야 할 일 (Action Items)

### 📝 정하늘 작가

| 순번 | 업무 | 마감일 | 우선순위 |
|------|------|--------|----------|
| 1 | 1화 캐릭터 관계 장면 보완 | 확인 필요 | 🔴 높음 |
| 2 | 1화 주인공 감정선 중심 재구성 | 확인 필요 | 🔴 높음 |
| 3 | 수정 원고 김민수 PD에 제출 | 확인 필요 | 🔴 높음 |

### 🎬 김민수 PD

| 순번 | 업무 | 마감일 | 우선순위 |
|------|------|--------|----------|
| 1 | 정하늘 작가와 1화 연출 추가 검토 | 확인 필요 | 🔴 높음 |
| 2 | 수정 완료 후 플랫폼 검수 제출 | 확인 필요 | 🔴 높음 |
| 3 | 매일 독자 피드백(댓글·리뷰) 수집 및 정리 | 런칭 후 | 🟡 중간 |

### 📱 이지은 마케팅팀장

| 순번 | 업무 | 마감일 | 우선순위 |
|------|------|--------|----------|
| 1 | 런칭 2주 전 티저 이미지 순차 공개 계획 수립 | 확인 필요 | 🔴 높음 |
| 2 | SNS 채널 캐릭터 소개 콘텐츠 연재 기획 | 확인 필요 | 🔴 높음 |
| 3 | 숏폼 영상 + 일러스트 콘텐츠 제작 | 확인 필요 | 🔴 높음 |
| 4 | 첫 주 SNS 광고 예산 집중 운영 | 런칭 후 | 🟡 중간 |

### 📊 박도현 마케터

| 순번 | 업무 | 마감일 | 우선순위 |
|------|------|--------|----------|
| 1 | 캐릭터 소개 카드뉴스 제작 | 확인 필요 | 🔴 높음 |
| 2 | 작가 인터뷰 콘텐츠 제작 | 확인 필요 | 🔴 높음 |
| 3 | 제작 비하인드 콘텐츠 제작 | 확인 필요 | 🔴 높음 |

### ✏️ 최유진 편집 담당

| 순번 | 업무 | 마감일 | 우선순위 |
|------|------|--------|----------|
| 1 | 1화 대사 가독성 수정 | 확인 필요 | 🔴 높음 |
| 2 | 1화 말풍선 배치 수정 (모바일 최적화) | 확인 필요 | 🔴 높음 |
| 3 | 2~8화 최종 검수 완료 | 확인 필요 | 🔴 높음 |

---

## ③ 위험 요소 (Risk)

| 위험 요소 | 영향도 | 설명 | 대응 방안 |
|-----------|--------|------|----------|
| 1화 수정 일정 지연 | 🔴 높음 | 플랫폼 검수 제출 지연 → 런칭 일정 영향 | 작가·PD 간 수정 일정 명확히 확정 필요 |
| 마케팅 콘텐츠 제작 일정 | 🔴 높음 | 런칭 2주 전 공개 목표 달성 불확실 | 콘텐츠별 제작 마감일 세부 수립 필요 |
| 플랫폼 검수 기간 | 🟡 중간 | 검수 소요 기간 미정 → 런칭 일정 변수 | 플랫폼 검수 예상 기간 확인 필요 |
| 초기 구독자 확보 | 🟡 중간 | 마케팅 효과 불확실성 | 첫 주 반응 분석 후 신속한 전략 조정 |

---

## ④ 다음 일정 (Next Schedule)

| 일정 | 담당자 | 상태 |
|------|--------|------|
| 1화 수정 완료 | 정하늘 작가 + 김민수 PD | 📅 확인 필요 |
| 플랫폼 검수 제출 | 김민수 PD | 📅 확인 필요 |
| 마케팅 콘텐츠 제작 완료 | 이지은 팀장 + 박도현 마케터 | 📅 확인 필요 |
| 런칭 2주 전 티저 공개 시작 | 이지은 팀장 | 📅 확인 필요 |
| 웹툰 런칭 | 전체 | 📅 확인 필요 |
| 런칭 후 첫 주 SNS 광고 운영 | 이지은 팀장 | 📅 런칭 후 |
| 매일 독자 피드백 수집 및 공유 | 김민수 PD | 📅 런칭 후 |

---

## ⑤ 확인 필요 사항

| 우선순위 | 확인 사항 |
|----------|-----------|
| 🔴 | 1화 수정 마감일은 언제인가? (플랫폼 검수 제출 일정과 연계) |
| 🔴 | 플랫폼 검수 소요 기간은 몇 일인가? |
| 🔴 | 정확한 런칭 예정일은 언제인가? |
| 🔴 | 마케팅 콘텐츠별 제작 마감일은 언제인가? (티저, 카드뉴스, 인터뷰, 비하인드) |
| 🔴 | SNS 광고 예산 규모는 얼마인가? |
| 🔴 | 독자 피드백 공유 방식은 어떻게 되는가? (일일 리포트 형식 등) |

---

## ⑥ 참석자별 최종 해야 할 일 (한 문장 요약)

| 참석자 | 최우선 업무 |
|--------|------------|
| 정하늘 작가 | 1화 캐릭터 관계와 주인공 감정선 중심으로 장면을 보완하여 수정 원고를 제시된 마감일까지 제출하세요. |
| 김민수 PD | 정하늘 작가와 1화 연출 검토를 완료한 후 플랫폼 검수에 제출하고, 런칭 후 매일 독자 피드백을 수집하여 작가에게 공유하세요. |
| 이지은 마케팅팀장 | 런칭 2주 전 티저 이미지 공개 계획을 수립하고 SNS 캐릭터 소개 콘텐츠 연재 일정을 확정하며, 첫 주 SNS 광고 예산을 집중 운영하세요. |
| 박도현 마케터 | 캐릭터 소개 카드뉴스, 작가 인터뷰, 제작 비하인드 콘텐츠를 제작하여 마케팅팀장에게 제시된 마감일까지 제출하세요. |
| 최유진 편집 담당 | 1화의 대사 가독성과 말풍선 배치를 모바일 환경에 최적화하여 수정하고, 2~8화 최종 검수를 완료하세요. |

<img width="942" height="1016" alt="스크린샷 2026-07-06 114023" src="https://github.com/user-attachments/assets/74bc823d-e125-4b5c-bc6c-0018449cf661" />
<img width="936" height="1008" alt="스크린샷 2026-07-06 114029" src="https://github.com/user-attachments/assets/8992abb9-0c67-49b0-ad20-c2f81df5a6bd" />
<img width="934" height="998" alt="스크린샷 2026-07-06 114035" src="https://github.com/user-attachments/assets/2613c3ca-91d9-48a5-9e5b-c093c101124a" />
<img width="938" height="997" alt="스크린샷 2026-07-06 114040" src="https://github.com/user-attachments/assets/0674d9cc-2312-4349-9830-b968045b19f7" />

이런 식의 결론이 나왔습니다. (v2)

환각 검증 역시 진행해봤고

<img width="411" height="187" alt="스크린샷 2026-07-06 114258" src="https://github.com/user-attachments/assets/080d64be-539b-436a-8a6e-230a3dbf93aa" />

# 🔍 사실 기반 검증 답변

| 번호 | 질문 | 결과 | 근거 |
|------|------|------|------|
| 1️⃣ | 최유진은 말풍선 수정을 주장했나? | ✅ **명확함** | 회의록에 **"최유진 편집: 대사 가독성/말풍선 배치 수정(모바일 최적화)"**라고 명시되어 있어 최유진이 말풍선 수정을 제안한 것으로 확인됨. |
| 2️⃣ | 정하늘은 초반 전개를 어떻게 하면 좋겠다고 생각했나? | ⚠️ **모호함** | 회의록에는 **"초반 전개 속도 완화, 자연스러운 관계 구축"**이 기록되어 있으나, 이것이 정하늘 작가의 직접 의견인지 회의 결과인지 명확하지 않음. |
| 3️⃣ | 댓글과 리뷰 데이터를 매일 취합하여 마케터에게만 보내는가? | ❌ **알 수 없음** | 회의록에는 **"댓글/리뷰 데이터 매일 취합 → 작가·PD 공유"**라고 명시되어 있어 마케팅팀만 공유한다는 내용은 확인되지 않음. |
| 4️⃣ | 이지은은 런칭 2주 전부터 티저 이미지를 순차적으로 공개하자고 했는가? | ⚠️ **모호함** | **"런칭 2주 전부터 티저 이미지 순차 공개"**는 회의 결과로 기록되어 있으나, 이지은이 직접 제안했는지는 확인되지 않음. |
| 5️⃣ | 「붉은 달의 계승자」는 김민수의 신작 판타지 웹툰인가? | ❌ **알 수 없음** | 회의록에는 **작가: 정하늘 / PD: 김민수**만 명시되어 있으며, 장르와 김민수가 작가라는 내용은 확인되지 않음. |

---

# 📋 검증 결과 요약

| 질문 | 판정 | 요약 |
|------|------|------|
| 1번 | ✅ 명확함 | 최유진이 말풍선 수정 제안 |
| 2번 | ⚠️ 모호함 | 정하늘의 직접 의견 여부 불명확 |
| 3번 | ❌ 알 수 없음 | 마케팅팀만이 아니라 작가·PD도 공유 |
| 4번 | ⚠️ 모호함 | 이지은의 직접 제안 여부 불명확 |
| 5번 | ❌ 알 수 없음 | 김민수는 PD, 장르는 미기재 |

<img width="753" height="655" alt="스크린샷 2026-07-06 114306" src="https://github.com/user-attachments/assets/6f133c3c-66a1-41a7-be51-80978787bf03" />
<img width="750" height="416" alt="스크린샷 2026-07-06 114312" src="https://github.com/user-attachments/assets/0b420f52-a949-4e9a-b333-5de44aeb7db8" />

이런 식의 결론이 나왔습니다.

다만 완벽하진 않은데

1번과 2 3 4번의 경우 정확하게 판별을 해냈지만

5번에서는 해매는 듯한 모습을 보였습니다.

따라서 이를 해결하기 위해선 새로운 설계가 필요해 보였고

이후 새로 만들어낸 프롬프트는 다음과 같습니다.


v1 : 내용 정리해줘

v2 :
당신은 10년차 프로젝트 매니저입니다.

1. 대화가 끝날 때까지 지금까지 확정된 모든 요구사항을 계속 유지하세요. 사용자가 새로운 조건을 추가하면 기존 조건은 유지하고, 변경을 명시한 부분만 수정하세요.

2. 회의록을 분석하여 아래 항목을 표 형식으로 작성하세요.
   - 결정사항
   - 해야 할 일(Action Items)
   - 위험 요소(Risk)
   - 다음 일정(Next Schedule)

3. 해야 할 일은 담당자별로 구분하여 작성하고, 특히 작가가 반드시 수행해야 하는 업무는 빠짐없이 포함하세요.

4. 정보가 부족하거나 회의록에서 확인되지 않는 내용은 절대 추측하지 마세요.
   - 확인 가능한 경우: 근거와 함께 답변
   - 확인 불가능한 경우: "알 수 없음" 또는 "확인 필요"로 표시
   - 정보가 부족하면 필요한 내용만 간단히 확인 질문하세요.

5. 최종 결과는 다음 순서로 출력하세요.
   ① 결정사항
   ② 해야 할 일
   ③ 위험 요소
   ④ 다음 일정
   ⑤ 확인 필요 사항
   ⑥ 참석자별 최종 해야 할 일(각 사람당 한 문장)



### 실행로그


   이 프롬프트를 이용해 7턴 이상의 대화로그 및 문맥유지 검증을 해보았습니다.

<img width="878" height="419" alt="스크린샷 2026-07-06 121551" src="https://github.com/user-attachments/assets/f04dc025-8b4c-4f6b-84b9-00294ec0e459" />
<img width="868" height="392" alt="스크린샷 2026-07-06 121604" src="https://github.com/user-attachments/assets/76ab24f2-608b-4c88-a9b8-1f09bedacb18" />
<img width="866" height="401" alt="스크린샷 2026-07-06 121610" src="https://github.com/user-attachments/assets/1d6891ca-ce1e-43e3-861f-fa4c3070cffa" />
<img width="854" height="404" alt="스크린샷 2026-07-06 121615" src="https://github.com/user-attachments/assets/c7740109-d722-4e6b-99fe-e16ab457d172" />
<img width="853" height="397" alt="스크린샷 2026-07-06 121620" src="https://github.com/user-attachments/assets/7a83c1ae-496a-47e4-9475-a1e1af96c833" />
<img width="866" height="258" alt="스크린샷 2026-07-06 121626" src="https://github.com/user-attachments/assets/ece5b8d6-6d8c-40c1-b679-9b76b8d0ac6e" />
<img width="864" height="414" alt="스크린샷 2026-07-06 121631" src="https://github.com/user-attachments/assets/679f4c0b-7838-41d0-b105-fc3ac2b025e9" />
<img width="861" height="369" alt="스크린샷 2026-07-06 121637" src="https://github.com/user-attachments/assets/447ce881-fc98-40f5-9ac8-b96fab4d16a6" />
<img width="853" height="378" alt="스크린샷 2026-07-06 121644" src="https://github.com/user-attachments/assets/da7f0f2e-4a36-4f43-a0df-9713dd5c1d9c" />
<img width="855" height="402" alt="스크린샷 2026-07-06 121649" src="https://github.com/user-attachments/assets/861c5273-89ca-4626-90d0-97a6a9330e7a" />
<img width="864" height="394" alt="스크린샷 2026-07-06 121655" src="https://github.com/user-attachments/assets/e76598ee-cf63-4f88-b456-96c598cc5bdc" />

보시다 시피 성공적으로 검증했습니다.


### 보너스 2 – 멀티모달 확장(업무 결과의 시각화)

<img width="1536" height="1024" alt="fa213dc8-9eb0-4f17-aac8-8466dd8df652" src="https://github.com/user-attachments/assets/290e1004-cdf9-49c6-8789-9e203c27af8b" />

